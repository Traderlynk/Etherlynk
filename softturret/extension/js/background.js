    ...
    if (pade.server && pade.domain && pade.username && pade.password)
    {
        ...
        pade.connection.connect(pade.username + "@" + pade.domain + "/" + pade.username, pade.password, function (status)
        {
            if (status === Strophe.Status.CONNECTED)
            {
                ...
                chrome.browserAction.setTitle({title: "Etherlynk ST - Connected"});
            ...
            {
                chrome.browserAction.setBadgeText({ text: "off" });
                chrome.browserAction.setTitle({title: "Etherlynk ST - Disconnected"});
            }
...
function setActiveContact(contact)
{
    ...
    chrome.browserAction.setTitle({title: "Etherlynk ST " + pade.activeContact.name + " (" + pade.activeContact.type + ")"});
}
