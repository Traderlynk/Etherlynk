function getEtherlynks()
{
		...
		for(var z = 0; z<lynkUI.calls.length; z++)
		{
			if (lynkUI.calls[z])
			{
				...
				if (jid == lynk.jid)
				{
					...
					notifyText(lynk.name, lynk.jid, null, [{title: "Answer Call?", iconUrl: chrome.extension.getURL("success-16x16.gif")}, {title: "Reject Call?", iconUrl: chrome.extension.getURL("forbidden-16x16.gif")}], function(notificationId, buttonIndex)
					{	
						...
						if (buttonIndex == 0)	// accept
						{
							...
							{
								etherlynk.leave(lynkUI.currentLynk.room);
							}
							
							etherlynk.join(room, lynk.server, lynk.domain);							
	...
	$(document).bind('ofmeet.media.joined', function(event, conf)
	{ 		
	...
	$(document).bind('ofmeet.media.left', function(event, conf)
	{ 		
	...
	{ 		
		if (lynkUI.currentLynk && lynkUI.currentLynk.room == conf.name)
		{
			try {
...
function handleButtonPress(button)
{
	...
	if (data.type == "call")
	{
		if (data.button[1] == null)
		{
			...
			data.lynk.room = data.lynk.etherlynk; 
			
			etherlynk.join(data.lynk.room);
			changeButton(button, "yellow", data.button[2]);
		...
		if (data.button[1] == "redflash")
		{
			...
			etherlynk.join(data.lynk.room, data.lynk.server, data.lynk.domain);
			lynkUI.currentLynk = data.lynk;
			
			chrome.notifications.clear(data.lynk.room, function(wasCleared)
			{
		...
		if (data.button[1] == "yellow")
		{
			etherlynk.leave(data.lynk.room);
		}		
	...
	{
		if (lynkUI.currentLynk) etherlynk.leave(lynkUI.currentLynk.room);
	}
...
function handleButtonHeld(button)
{
	...
	if (data.type == "call")
	{
		if (data.button[1] == "yellow" || data.button[1] == "red" || data.button[1] == "green")
		{
			etherlynk.leave(data.lynk.room);
		}		
	}
...
{	
	...
	if (slider == 56)		// handset slider
	{
		...
		{
			var audio = document.getElementById("remoteAudio-" + lynkUI.currentLynk.room);
